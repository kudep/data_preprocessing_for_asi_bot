...
def get_data(path_to_files):
    '''
    # Get dataset path to fales => list[list,..]
    dataset = preprocessing.get_data(path_to_files)
    '''
    ...
    for (dirpath, dirnames, filenames) in os.walk(path_to_files):
        ...
        break
    output_data = []
    for txt_file in bar(files):
        with open(os.path.join(path_to_files, txt_file)) as file_disc:
            output_data.append(file_disc.readlines())
    return output_data


def save_data(input_data, path_to_files):
    '''
    # Get dataset path to fales => list[list,..]
    dataset = preprocessing.get_data(path_to_files)
    '''
    bar = progressbar.ProgressBar()
    bar.init()
    for chat_id, chat in bar(enumerate(input_data)):
        if len(chat) < 2 : continue
        with open(os.path.join(path_to_files, str(chat_id) + '.txt'), 'wt') as file_disc:
            for replie in chat:
                file_disc.write(replie.strip() + '\n')

def tags_off(input_data):
    '''
    # Delete tags <\d+?> list[list,..] => list[list,..]
    dataset = preprocessing.tags_off(dataset)
    '''
    bar = progressbar.ProgressBar()
    bar.init()
    output_data = []
    for chat in bar(input_data):
        output_data.append([re.sub(r'<\d+\w?>', '<next>', line) for line in chat])
    return output_data
